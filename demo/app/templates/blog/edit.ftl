...
<div class="container">
    <h2>blog create</h2>
    <hr>
    <form action="/blog/doUpdate" method="post">
        <input type="hidden" name="blogId" value="${(context._form.blogId)!}"/>
                            ...
                            <#list inner_map?keys as key>
                                <label class="radio">
                                    <input type="radio" name="blogStatus" value="${key}"
                                        <#if context._form.blogStatus?exists && equal(key,context._form.blogStatus) >
                                           checked
                                        </#if>
                                            >
                                ${inner_map[key]}
